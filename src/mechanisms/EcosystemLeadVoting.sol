...
import {QuadraticVotingMechanism, AllocationConfig} from "@octant-core/mechanisms/mechanism/QuadraticVotingMechanism.sol";
...
contract EcosystemLeadVoting is QuadraticVotingMechanism {
    ...
    /**
     * @notice Hook to authorize proposal creation
     * @dev Allows proposals from keeper/management OR from our own proposal functions
     * @param proposer Address attempting to propose (will be proxy during delegatecall from our functions)
     * @return authorized True if allowed to propose
     */
    function _beforeProposeHook(address proposer) internal view virtual override returns (bool) {
        // Allow if proposer is keeper/management
        if (super._beforeProposeHook(proposer)) {
            return true;
        }

        // Allow if proposer is this contract (called from our proposal functions)
        // This is safe because we've already validated the caller is an ecosystem lead in the proposal functions
        return proposer == address(this);
    }
    ...
    function proposeContributorFunding(
        ...
        // Check if caller is ecosystem lead
        if (ecosystemLeadNFT.balanceOf(msg.sender) == 0) {
            revert NotAnEcosystemLead(msg.sender);
        }
        ...
        // Simple description without complex string encoding
        string memory description = string(abi.encodePacked("Contributor Funding: ", reason));
    ...
    function proposeNewLead(address nominee, string calldata reason) external returns (uint256 pid) {
        ...
        // Simple description without complex string encoding
        string memory description = string(abi.encodePacked("New Ecosystem Lead: ", reason));
    ...
    /**
     * @notice Convert uint256 to string (simplified)
