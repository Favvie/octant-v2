...
contract AaveV3YieldDonatingOperationTest is AaveV3YieldDonatingSetup {
    ...
    function test_aaveIntegration() public view {
    ...
    function test_multipleUsersDepositAndWithdraw() public {
        ...
        // All users withdraw - calculate shares before pranking
        uint256 userShares = strategy.balanceOf(user);
        uint256 user2Shares = strategy.balanceOf(user2);
        uint256 user3Shares = strategy.balanceOf(user3);
        ...
        vm.prank(user3);
        strategy.redeem(user3Shares, user3, user3);
