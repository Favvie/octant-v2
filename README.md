...
**Gas Costs**:
- Registration: ~2,000-3,000 gas (Merkle proof verification)
- Lookup: ~100 gas (simple mapping)
- No reliance on Gitcoin Passport or centralized APIs
...
### TypeScript-Based Deployment

Deploy using TypeScript scripts with ethers.js:
...
make test-contract contract=AaveV3YieldDonating # Run specific test
make trace                                      # Verbose output
make gas                                        # Gas report
make coverage                                   # Code coverage

# Inspection
make inspect contract=AaveV3YieldDonatingStrategy # Storage layout
```
...
### Example 1: ERC4626 Vault Strategy
...
contract ERC4626YieldDonatingStrategy is YieldDonatingStrategy {
    ...
    function _deployFunds(uint256 _amount) internal override {
        IERC20(asset).approve(address(yieldVault), _amount);
        yieldVault.deposit(_amount, address(this));
    }

    function _freeFunds(uint256 _amount) internal override {
        uint256 shares = yieldVault.convertToShares(_amount);
...
### Example 2: Compound V3 Strategy
...
contract CompoundV3YieldDonatingStrategy is YieldDonatingStrategy {
    IComet public comet;
    ...
    function _freeFunds(uint256 _amount) internal override {
        comet.withdraw(address(asset), _amount);
    }
    ...
    function _harvestAndReport()
        ...
        returns (uint256 _totalAssets)
    {
        // Get balance in Compound (may include interest)
        uint256 compoundBalance = comet.balanceOf(address(this));

        // Get idle assets
        uint256 idle = IERC20(asset).balanceOf(address(this));

        _totalAssets = compoundBalance + idle;
    }
}
```

### Example 3: Lido Liquid Staking Strategy
...
contract LidoYieldDonatingStrategy is YieldDonatingStrategy {
    ILido public lido;

    function _deployFunds(uint256 _amount) internal override {
        // ETH-based, convert and stake
        (bool success, ) = address(lido).call{value: _amount}("");
        require(success);
    }

    function _freeFunds(uint256 _amount) internal override {
        // Request withdrawal (stETH → ETH)
        uint256 shares = lido.sharesOf(address(this));
        lido.requestWithdrawal(shares, address(this));
    }
        ...
        returns (uint256 _totalAssets)
    {
        ...
        uint256 ethIdle = address(this).balance;

        // Note: stETH is typically used for ETH backing
        _totalAssets = stETHBalance + ethIdle;
    }
}
```
...
#### 1. Tests Fail with "Fork Initialization Error"

**Symptom**:
```
Error: Failed to initialize fork
```
...
# Try using Alchemy or Infura instead of local node
ETH_RPC_URL=https://eth-mainnet.alchemyapi.io/v2/YOUR_KEY forge test
```

#### 2. Strategy Returns Wrong Profit Amount

**Symptom**:
```
Expected profit: 1000, got 500
```

**Causes & Fixes**:
```solidity
// WRONG: Only counting vault assets
function _harvestAndReport() internal override returns (uint256) {
    uint256 vaultAssets = yieldVault.convertToAssets(
        yieldVault.balanceOf(address(this))
    );
    return vaultAssets; // ❌ Missing idle assets!
}

// CORRECT: Include both deployed and idle
function _harvestAndReport() internal override returns (uint256) {
    uint256 vaultAssets = yieldVault.convertToAssets(
        yieldVault.balanceOf(address(this))
    );
    uint256 idle = IERC20(asset).balanceOf(address(this));
    return vaultAssets + idle; // ✅ Includes all assets
}
```

#### 3. Deployment Script Hangs

**Symptom**:
```
Waiting for transaction 0x... (this can take a while)
```

**Solution**:
```bash
# Add timeout and verbosity
forge script script/DeployFactory.s.sol \
  --broadcast \
  --timeout 300 \
  -vvv

# Or use cast directly for debugging
cast send 0x... "function()" --private-key $KEY
```

#### 4. "Insufficient Balance" on Withdrawal

**Symptom**:
```
Error: Withdrawal amount exceeds available liquidity
```

**Causes**:
- Yield source has liquidity constraints (e.g., Aave pool depleted)
- Tenderly fork is stale and doesn't reflect real state
- Bug in `_freeFunds()` implementation

**Solution**:
```bash
# Use fresh mainnet fork
FORK=true FORK_BLOCK=latest forge test

# Check protocol liquidity
cast call $AAVE_POOL "getReserveData(address)" $ASSET

# Verify _freeFunds works correctly
forge test --match-test testFreeFunds -vvv
```

#### 5. "Invalid Merkle Proof" for Contributor Registry
...
**Causes**:
- Merkle root doesn't match the proof data
- Contributors list changed without updating root
- User provided wrong proof path

**Solution**:
```bash
# Regenerate Merkle tree from current contributors
npm run generate:merkle-tree

# Update contract with new root
cast send $REGISTRY "setMerkleRoot(bytes32)" 0x...

# Verify contributor data matches
npm run verify:contributor -- --address 0x...
```

#### 6. APR Oracle Returns 0
...
- Profit calculation is negative (losses)

**Solution**:
```bash
# Wait for at least one report cycle
# Run report multiple times
forge script script/RunReport.s.sol --broadcast

# Manually verify APR calculation
uint256 apr = oracle.calculateApr(strategy);
```
...
forge inspect YieldDonatingStrategy storage-layout

# Get detailed trace of failed test
forge test --match-test testFailed -vvv

# Check gas usage
forge test --gas-report

# Verify function selectors
cast sig-event "Approval(address indexed owner, address indexed spender, uint256 value)"

# Decode transaction data
cast call-data "deposit(uint256)" 1000e6
```
...
```bash
git clone git@github.com:YOUR_USERNAME/octant-v2-strategy-foundry-mix.git
cd octant-v2-strategy-foundry-mix
```
...
```bash
git commit -m "feat(strategy): add Yearn V3 yield strategy"
git commit -m "test(aave): add test for emergency shutdown"
git commit -m "docs(readme): update contributor registry section"
```
...
- **New Strategies**: Implement support for additional protocols
- **Bug Fixes**: Fix issues in existing code
- **Documentation**: Improve README, add examples, fix typos
- **Tests**: Add test coverage for edge cases
- **Gas Optimization**: Reduce contract deployment/execution costs

### Code Standards

- **Solidity**: Follow OpenZeppelin style guide
- **Testing**: Maintain >80% code coverage
- **Gas**: Use `forge test --gas-report` to check usage
- **Linting**: Run `solhint` before committing
- **Formatting**: Use `prettier` with `.prettierrc` settings
...
- ✅ Pass gas benchmarks
- ✅ Work with mainnet fork tests

```bash
# Complete check before submitting PR
make build      # Compile without errors
make test       # Pass all tests
make gas        # Check gas usage
make coverage   # Maintain coverage
```
...
- **Yearn Strategies**: https://docs.yearn.finance/
...
- **Octant GitHub**: https://github.com/golemfoundation/octant-v2-core
...
- **Discord**: [Octant Community](https://discord.gg/octant)
...
|---------|------------------|----------------------|
| **Performance Fees** | Charges 10-20% fees to LPs | NO fees - 100% yield donated |
| **Profit Distribution** | Kept by strategy/protocol | Minted as shares to dragonRouter |
| **Use Case** | Maximize LP returns | Fund public goods & open-source |
| **Loss Handling** | Users bear losses | Optional dragonRouter share burning |
| **Governance** | Single entity (team) | NFT-based DAO with quadratic voting |
| **Transparency** | Off-chain accounting | Full on-chain auditability |
...
```bash
# Setup
git clone <repo> && cd octant-v2-strategy-foundry-mix
...
npm run deploy:strategy       # Strategy only

# Monitoring
npm run check:distribution    # Weekly distribution status
...
cast call <ADDRESS> <FUNCTION> # Query contract
```

---

**Last Updated**: November 2024
**Maintained By**: Octant Team
**Status**: Production Ready

For questions or issues, please open a [GitHub Issue](https://github.com/golemfoundation/octant-v2-strategy-foundry-mix/issues).
